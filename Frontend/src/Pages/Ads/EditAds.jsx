...
import { AdsEditAtom } from "./MyAds";

export default function EditAds() {
  const navigate = useNavigate();
  const [adsID] = useAtom(AdsEditAtom);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8000/adsdata/", {
        ...
        hashtagRef.current.value = response.data.hashtag;
